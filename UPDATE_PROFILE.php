...
$uid = $_SESSION['uid'];
$res = mysqli_query($conn, "SELECT * FROM `admin_user` WHERE id='$uid'");
...
if (isset($_POST['updateAcount'])) {
    if (empty($_FILES['img']['name'])) {
        ...
        $upq = mysqli_query($conn, "
            UPDATE `admin_user` 
            SET `contact`='" . mysqli_real_escape_string($conn, $_POST['contact']) . "' 
            WHERE id = '" . $thisuser['id'] . "'
        ");
        if ($upq) {
            $msg = "Contact updated successfully! _success";
        } else {
            $msg = "Unknown Error! _danger";
        }
    } else {
        // Handle image + contact update
        $target_dir = "IMGS/";
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
                $upq = mysqli_query($conn, "
                    UPDATE `admin_user` 
                    SET `img` = '" . mysqli_real_escape_string($conn, $target_file) . "',
                        `contact`='" . mysqli_real_escape_string($conn, $_POST['contact']) . "' 
                    WHERE id = '" . $thisuser['id'] . "'
                ");
                if ($upq) {
                    $msg = "Profile updated successfully! _success";
    ...
    <div class="page-wrapper">
        <header class="top-header">
            ...
            <div class="user-info">
                ...
                <?php } else { ?>
                    <img src="IMGS/user.png" alt="Default User Image" />
                <?php } ?>
                    ...
                    <form action="UPDATE_PROFILE.php" method="POST" enctype="multipart/form-data">
                        <div class="row align-items-center mb-3">
                            ...
                            <div class="col-md-6 text-center">
                                ...
                                <?php } else { ?>
                                    <img src="IMGS/user.png" id="profile-img" alt="Profile Image" class="rounded-circle" />
                                <?php } ?>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" id="name" name="name" required class="form-control"
                                value="<?= htmlspecialchars($thisuser['name']) ?>" readonly />
                        </div>
                        <div class="mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" id="email" name="email" required class="form-control"
                                value="<?= htmlspecialchars($thisuser['email']) ?>" readonly />
                        </div>
                        <div class="mb-3">
                            <label for="contact" class="form-label">Contact</label>
                            <input type="text" id="contact" name="contact" required class="form-control"
                                value="<?= htmlspecialchars($thisuser['contact']) ?>" />
                        </div>
                        <div class="text-end">
